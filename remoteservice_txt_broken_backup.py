...
def parse_irnetbox_file(filepath):
    ...
    while i < len(lines):
        ...
        if 'MOD_SIG' in line or 'DMOD_SIG' in line:
            ...
            if 'DMOD_SIG' in line and len(parts) >= 5:
                signal_name = parts[0]
                # For DMOD_SIG, parts[2] is signal1/signal2, parts[3] is max_lengths, parts[4] is hex_data
                max_lengths = int(parts[3])
                hex_data = parts[4].strip()
                # Create unique key for dual signals
                signal_type = parts[2]  # 'signal1' or 'signal2'
                key = f"{signal_name}_{signal_type}"
            elif 'MOD_SIG' in line and len(parts) >= 4:
                signal_name = parts[0]
                # For MOD_SIG, parts[2] is max_lengths, parts[3] is hex_data
                max_lengths = int(parts[2])
                hex_data = parts[3].strip()
                key = signal_name
            else:
                print(f"Skipping unparseable line {i+1}: {line}")
                i += 1
                continue
            
            # For this simple format, hex data is all on one line - no need for continuation logic
            # Just process the hex_data we already have
        if 'DMOD_SIG' in line or 'MOD_SIG' in line:
            ...
            if len(parts) >= 4:
                ...
                while i < len(lines):
                    ...
                    # Check if this is the start of a new signal (numeric name followed by MOD_SIG or DMOD_SIG)
                    if next_line and ('\tMOD_SIG\t' in next_line or '\tDMOD_SIG\t' in next_line):
                        break
                    # Check if this is a hex data continuation line
                    # It should be pure hex data (no signal keywords) and reasonably long
                    if (next_line and 
                        all(c in '0123456789ABCDEFabcdef ' for c in next_line) and
                        len(next_line.replace(' ', '')) > 10):
